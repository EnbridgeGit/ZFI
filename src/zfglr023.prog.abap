REPORT ZFGLR023 NO STANDARD PAGE HEADING LINE-SIZE 90
                                         LINE-COUNT 58.

************************************************************************
*  Author:      Mohammad T. Khan                                       *
*  Date:        April 2005.                                            *
*  Issue Log:   1097                                                   *
*  Description:                                                        *
*  This program generates a report for accounts with no input tax.     *
*  It also allows to report on other G/L Accounts.                     *
********* *************** ******************* *********** **************
*  This program has been cloned from ZFGLR009 and changes are          *
*  made as per issue log: 1097.                                        *
*  Run time has been optimized as well.                               *
*                                                                      *
*                                                                      *
*                                                                      *
************************************************************************
TABLES: BKPF, "Accounting Document Header
        BSEG, "Accountind document segment
        SKAT, "G/L accounts master records - Chart of Accounts
        T001. "Company Codes
DATA:
      POST1      TYPE D,                       "First Date Range
      POST2      LIKE POST1,                   "Second Date Range
      LN_CNTR    TYPE I,                       "Line Counter
      W_DOLLAR   TYPE P DECIMALS 2.

DATA:
     BEGIN OF WTAB OCCURS 500,
       BLART LIKE BKPF-BLART,                  "Document Type
       BELNR LIKE BSEG-BELNR,                  "Document Reference No.
       MWSKZ LIKE BSEG-MWSKZ,                  "Tax Code
       HKONT LIKE BSEG-HKONT,                  "G/L Account
       WRBTR(15) TYPE C,                       "Amount
*       WRBTR LIKE BSEG-WRBTR,                  "Amount
       BUKRS LIKE BSEG-BUKRS,                  "Company Code
       BUDAT LIKE BKPF-BUDAT,                  "Posting Date
       GJAHR LIKE BSEG-GJAHR,                  "Fiscal Year
      END OF WTAB,

     BEGIN OF TEMPTAB OCCURS 500,
       HKONT LIKE BSEG-HKONT,                  "G/L Account
       BELNR LIKE BSEG-BELNR,                  "Document Reference No.
      END OF TEMPTAB.

* This first block produces a border like frame around the following: *
SELECTION-SCREEN BEGIN OF BLOCK
    INTRO WITH FRAME.

SELECTION-SCREEN BEGIN OF LINE.
SELECTION-SCREEN: COMMENT 26(31) TEXT-000 MODIF ID ABC.
SELECTION-SCREEN END OF LINE.

SELECTION-SCREEN BEGIN OF LINE.
SELECTION-SCREEN ULINE  26(30).
SELECTION-SCREEN END OF LINE.

SELECTION-SCREEN BEGIN OF LINE.
SELECTION-SCREEN: COMMENT 1(14) TEXT-002 MODIF ID BBC.
PARAMETERS:
  P_COMPNY LIKE T001A-BUKRS       "Company Code
           OBLIGATORY MODIF ID BBC.
SELECTION-SCREEN END OF LINE.
SELECTION-SCREEN SKIP.

* The following third block produces a frame within the first block. *
SELECTION-SCREEN BEGIN OF BLOCK
    PERIOD WITH FRAME TITLE TEXT-003.

SELECTION-SCREEN BEGIN OF LINE.
SELECTION-SCREEN: COMMENT 31(10) TEXT-005 MODIF ID ABC.
SELECTION-SCREEN: COMMENT 56(10) TEXT-005 MODIF ID ABC.
SELECTION-SCREEN END OF LINE.

*SELECTION-SCREEN SKIP.

SELECTION-SCREEN BEGIN OF LINE.
SELECTION-SCREEN: COMMENT 1(29) TEXT-006.
PARAMETERS: P_DAY1(2)   DEFAULT '01'           OBLIGATORY MODIF ID BBC,
            P_MONTH1(2) DEFAULT '01'           OBLIGATORY MODIF ID BBC,
            P_YEAR1(4)  DEFAULT SY-DATUM(4)    OBLIGATORY MODIF ID BBC.
SELECTION-SCREEN: COMMENT 42(13) TEXT-007.
PARAMETERS: P_DAY2(2)   DEFAULT SY-DATUM+4(2)  OBLIGATORY MODIF ID BBC,
            P_MONTH2(2) DEFAULT SY-DATUM+6(2)  OBLIGATORY MODIF ID BBC,
            P_YEAR2(4)  DEFAULT SY-DATUM(4)    OBLIGATORY MODIF ID BBC.
SELECTION-SCREEN END OF LINE.
SELECT-OPTIONS: S_DOCTYP FOR BKPF-BLART NO INTERVALS
                          MODIF ID BBC.          "Document Types
*SELECTION-SCREEN END OF LINE.

* This statement ends the third block. *
SELECTION-SCREEN END OF BLOCK PERIOD.

SELECTION-SCREEN SKIP.

* This fourth block produces a frame around the first block. *
SELECTION-SCREEN BEGIN OF BLOCK
     ACCNT WITH FRAME TITLE TEXT-004.


SELECT-OPTIONS:  S_GLACCT FOR BSEG-HKONT NO INTERVALS
                          DEFAULT '0000256950'   MODIF ID BBC.

* This statement ends the fourth block. *
SELECTION-SCREEN END OF BLOCK ACCNT.



* This statement ends the first block. *
SELECTION-SCREEN END OF BLOCK INTRO.

* The following will highlight the screen's output for certain texts. *
AT SELECTION-SCREEN OUTPUT.
 LOOP AT SCREEN.
  CHECK SCREEN-GROUP1 = 'ABC'.
  SCREEN-INTENSIFIED = '1'.
  MODIFY SCREEN.
 ENDLOOP.

* initialization
INITIALIZATION.
IF SY-MANDT+2(1) = '1'.
  P_COMPNY = 'UEC'.
ELSEIF  SY-MANDT+2(1) = '0'.
  P_COMPNY = 'UGL'.
ENDIF.

*TOP OF PAGE
TOP-OF-PAGE.
  WRITE: /1 TEXT-RPT, SY-REPID INTENSIFIED ON,
         30 T001-BUTXT INTENSIFIED ON,
         63 TEXT-DTE, SY-DATUM, TEXT-AMP, SY-UZEIT.

  WRITE: /1 TEXT-CLT, SY-MANDT UNDER SY-REPID,
                                        SY-SYSID.
  WRITE: 20 TEXT-001.
  WRITE: 79 TEXT-PGE, SY-PAGNO.
       SKIP 1.
  ULINE.


*********************BEGINNING OF MAIN PROGRAM *************************

START-OF-SELECTION.

* get company code text
   SELECT SINGLE BUTXT FROM T001 INTO T001-BUTXT WHERE BUKRS = P_COMPNY.

 PERFORM INITIALIZE.
 PERFORM GET_PAY.
 PERFORM GET_GST.
 PERFORM PROC_INFO.
END-OF-SELECTION.

FORM INITIALIZE.
 REFRESH: WTAB, TEMPTAB.
 CLEAR:   WTAB, TEMPTAB.
 MOVE '60' TO LN_CNTR.


 CONCATENATE P_YEAR1 P_DAY1 P_MONTH1 INTO POST1. "Combines date to post1
 CONCATENATE P_YEAR2 P_DAY2 P_MONTH2 INTO POST2. "Combines date to post2
ENDFORM.


FORM GET_PAY.

 SELECT BELNR BLART BUDAT BUKRS GJAHR
   INTO (BKPF-BELNR, BKPF-BLART, BKPF-BUDAT, BKPF-BUKRS, BKPF-GJAHR)
   FROM BKPF
  WHERE BLART IN S_DOCTYP AND BUKRS = P_COMPNY
    AND BUDAT BETWEEN POST1 AND POST2
  ORDER BY BELNR BLART BUDAT.

*   SELECT BUKRS BELNR GJAHR MWSKZ HKONT WRBTR
   SELECT BUKRS BELNR GJAHR MWSKZ HKONT WRBTR SHKZG     "New 19/05/2005
     INTO (BSEG-BUKRS, BSEG-BELNR, BSEG-GJAHR, BSEG-MWSKZ,
           BSEG-HKONT, BSEG-WRBTR, BSEG-SHKZG)          "New 19/05/2005
*           BSEG-HKONT, BSEG-WRBTR BSEG-SHKZG)
     FROM BSEG
    WHERE BELNR = BKPF-BELNR
      AND BUKRS = BKPF-BUKRS
      AND GJAHR = BKPF-GJAHR.

      PERFORM POP_WTAB.
   ENDSELECT.

 ENDSELECT.
ENDFORM.


FORM GET_GST.
 SORT: WTAB BY BELNR.
 LOOP AT WTAB.
  IF WTAB-HKONT IN S_GLACCT.
     MOVE: WTAB-BELNR TO TEMPTAB-BELNR,
           WTAB-HKONT TO TEMPTAB-HKONT.
     APPEND TEMPTAB.
  ENDIF.
 ENDLOOP.
ENDFORM.

FORM POP_WTAB.
 CLEAR: WTAB.
 MOVE:  BKPF-BLART TO WTAB-BLART,
*       BSEG-WRBTR TO WTAB-WRBTR,
        BSEG-MWSKZ TO WTAB-MWSKZ,
        BSEG-BUKRS TO WTAB-BUKRS,
        BKPF-BUDAT TO WTAB-BUDAT,
        BSEG-GJAHR TO WTAB-GJAHR,
        BSEG-BELNR TO WTAB-BELNR,
        BSEG-HKONT TO WTAB-HKONT.
 IF BSEG-SHKZG = 'H'.                                 "New 19/05/2005
    W_DOLLAR = BSEG-WRBTR * -1.                       "New 19/05/2005
 ELSE.
    W_DOLLAR = BSEG-WRBTR.
 ENDIF.
    MOVE W_DOLLAR TO WTAB-WRBTR.
    PERFORM PUT_SIGN_IN_FRONT CHANGING WTAB-WRBTR.    "New 19/05/2005
*    WTAB-WRBTR = W_DOLLAR.                            "New 19/05/2005
* ELSE.                                                "New 19/05/2005
*    WTAB-WRBTR = BSEG-WRBTR.                          "New 19/05/2005
* ENDIF.                                               "New 19/05/2005

 APPEND WTAB.
ENDFORM.

*----------------------------------------------------------------------*
*       FORM PUT_SIGN_IN_FRONT.                        "New 19/05/2005 *
*----------------------------------------------------------------------*

FORM PUT_SIGN_IN_FRONT CHANGING VALUE.
  DATA: TEXT1(1) TYPE C.

  SEARCH VALUE FOR '-'.
  IF SY-SUBRC = 0 AND SY-FDPOS <> 0.
    SPLIT VALUE AT '-' INTO VALUE TEXT1.
    CONDENSE VALUE.
    CONCATENATE '-' VALUE INTO VALUE.
  ELSE.
    CONDENSE VALUE.
  ENDIF.
ENDFORM.

FORM PROC_INFO.
 LOOP AT WTAB.
   READ TABLE TEMPTAB WITH KEY BELNR = WTAB-BELNR.
   IF WTAB-BELNR = TEMPTAB-BELNR.
     CONTINUE.
   ELSE.
   AT NEW BELNR.
    IF LN_CNTR >= 59.
       PERFORM WRT_HDG.
    ENDIF.
      WRITE: /1 WTAB-BELNR, 12 WTAB-BLART.
      LN_CNTR = LN_CNTR + 1.
   ENDAT.
     SELECT SINGLE * FROM SKAT WHERE SPRAS = 'EN'
                                 AND KTOPL = 'COAT'
                                 AND SAKNR = WTAB-HKONT.
     IF LN_CNTR >= 59.
        PERFORM WRT_HDG.
     ENDIF.
    WRITE: /22 WTAB-BUDAT, 34 WTAB-MWSKZ, 40 WTAB-HKONT,
              53 SKAT-TXT20, 75 WTAB-WRBTR RIGHT-JUSTIFIED.
      LN_CNTR = LN_CNTR + 1.
   ENDIF.
 ENDLOOP.
ENDFORM.


* This routine writes the sub headings. *
FORM WRT_HDG.
 NEW-PAGE.
 CLEAR LN_CNTR.
 FORMAT INTENSIFIED ON.
 SKIP.
 WRITE: /1  TEXT-008, 15 P_COMPNY.
 WRITE: 20  TEXT-006, 33 P_DAY1, 35 '/', 36 P_MONTH1,
        38 '/', 39 P_YEAR1.
 WRITE: 45  TEXT-018, 49 P_DAY2, 51 '/', 52 P_MONTH2, 54 '/'.
 WRITE: 55 P_YEAR2, 71 TEXT-009.
 ULINE: /.
 WRITE: /1 TEXT-016, 12 TEXT-016, 22 TEXT-027, 34 TEXT-012, 40 TEXT-010.
 WRITE: 53 TEXT-011, 83 TEXT-011.
* SKIP.
 WRITE: /1 TEXT-017, 12 TEXT-014, 22 TEXT-019, 34 TEXT-013, 40 TEXT-011.
 WRITE: 53 TEXT-015, 84 TEXT-021.
 ULINE: /.
 MOVE '11' TO LN_CNTR.
 FORMAT INTENSIFIED OFF.
ENDFORM.
