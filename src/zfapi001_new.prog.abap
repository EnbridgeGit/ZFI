REPORT zfapi001_new MESSAGE-ID zs.
************************************************************************
...
************************************************************************
* ---------------------- CHANGE LOG ------------------------------------
* 2011/09/22 gymana   COG TR804 - Reset Customer/Premise formatting and
*                     restore payment advice information.
* 2011/06/24 gymana   COG TR804 - Created this new copy as part of the
*                     Cost of Gas project (This program will send Banner
*                     data to screen 301
...
*=======================================================================
*AT SELECTION-SCREEN.                                            "TR804
*  PERFORM CHECK_FILE.                                           "TR804
AT SELECTION-SCREEN.
...
*-----------------------------------------------------------------------
FORM open_file.                                                   "TR804
                                                                  "TR804
  DATA: msg(100).         "open file - system message             "TR804
                                                                  "TR804
  OPEN DATASET p_infile FOR INPUT IN TEXT MODE MESSAGE msg.       "TR804
  IF ( sy-subrc <> 0 ).                                           "TR804
    MESSAGE ID 'ZS' TYPE 'E' NUMBER '002' WITH p_infile msg.      "TR804
  ENDIF.                                                          "TR804
                                                                  "TR804
ENDFORM.                    "OPEN_FILE                            "TR804
...
*  TR804 - Logical file removed and replaced with flat file read
*-----------------------------------------------------------------------
*FORM CHECK_FILE.
*
...
*  PERFORM bdc_screen USING 'SAPLKACB' '002'.                   "TR804
*  PERFORM bdc_field  USING 'BDC_OKCODE' '/8'.                  "TR804
...
*   account line item.
*-----------------------------------------------------------------------
FORM add_gl_line_item.                                          "TR804
                                                                "TR804
  PERFORM bdc_screen USING 'SAPMF05A' '301'.                    "TR804
  PERFORM bdc_field  USING 'BSEG-WRBTR' zbseg-wrbtr.            "TR804
  PERFORM bdc_field  USING 'BSEG-ZFBDT' sy-datum.               "TR804
  PERFORM bdc_field  USING 'BSEG-SGTXT' zbseg-sgtxt.            "TR804
  PERFORM bdc_field  USING 'BSEG-SKFBT' '0.00'.                 "TR804
                                                                "TR804
ENDFORM.                    "ADD_GL_LINE_ITEM                   "TR804
