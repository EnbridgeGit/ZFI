...
* -----------  ----------  ----------  ------------------------------- *
* 15-JUL-2022  DADIM   D30K932215  CHG0256185 - Update payment advice  *
*                                  information text field from file    *

REPORT  zfapi60_guardian_incoming_inv MESSAGE-ID zs.
...
*         meins  TYPE bseg-meins, "Unit
         text   TYPE char75,                "Added by DADIM
       END OF ty_acc_data.
...
CONSTANTS:
        ...
        gc_x             TYPE char1              "X / Yes / True       "
                         VALUE 'X',
        gc_onetimevendor TYPE lfa1-lifnr VALUE 'OTGUAR'.
...
PARAMETERS:  p_blart TYPE bkpf-blart DEFAULT 'GD', " OBLIGATORY ,
             p_ident TYPE char04 DEFAULT 'GUAR',
...
*    ENDIF.
    ...
    ELSE.
      CONCATENATE gs_data-col19 gs_data-col20
                      INTO gs_acc_data-xblnr.
    ENDIF.
...
*    gs_acc_data-bktxt = gs_data-col3.
    ...
    ENDIF.
*Start by DADIM for CHG0256185
    gs_acc_data-text = gs_data-col27.
*End by DADIM for CHG0256185
    "------------------
...
*Start by DADIM for CHG0256185
  ...
  CONSTANTS : lc_id TYPE tdid VALUE '0003',
              lc_language TYPE tdspras VALUE 'E',
...
*Start by DADIM for CHG0256185
      IF gs_doc_data-text IS NOT INITIAL.

        lv_name+0(4) = gs_return-message_v2+10(4).
        lv_name+4(10) = gs_return-message_v2+0(10).
        lv_name+14(4) = gs_return-message_v2+14(4).
        ls_lines-tdline = gs_doc_data-text.
        APPEND ls_lines TO lt_lines.
*Create Text
        CALL FUNCTION 'CREATE_TEXT'
          EXPORTING
            fid         = lc_id
            flanguage   = lc_language
            fname       = lv_name
            fobject     = lc_object
            save_direct = gc_x
          TABLES
