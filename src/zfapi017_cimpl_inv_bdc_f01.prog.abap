...
*15/10/2020 D30K930713 AKMADASU CHG0191529- Team connect Interface tax *
*           D30K930715                      code changes               *
*           D30K930718                                                 *
*           D30K930720                                                 *
*           D30K930721                                                 *
*           D30K930723                                                 *
...
** Logic for Checking if Duplication exists in posting entries.
  ...
  PERFORM f_process_duplication. " Added BY AKMADASU FOR CHG0191529
...
* Document header record
      WHEN   '1'.
        ...
        CLEAR                          lwa_bbkpf.
        CLEAR:                         gv_xblnr,gv_bukrs. " Added BY AKMADASU FOR CHG0191529
        MOVE     lwa_rfbibl00       TO lwa_bbkpf.
        gv_xblnr = lwa_bbkpf-xblnr.              " Added BY AKMADASU FOR CHG0191529
        gv_bukrs = lwa_bbkpf-bukrs.              " Added BY AKMADASU FOR CHG0191529
...
* Document line item record
      WHEN   '2'.
        ...
        CLEAR    lv_newbk.
**--START OF CHNAGES BY AKMADASU FOR CHG0191529
        IF lwa_bbseg-WRBTR is INITIAL or lwa_bbseg-WRBTR = '0.00'.
          CONTINUE.
        ENDIF.
**--END OF CHNAGES BY AKMADASU FOR CHG0191529
...
FORM f_format_bdc_vendor_item
  ...
  DATA:    lv_lifnr  TYPE lifnr,
           lv_MWSKZ  TYPE mwskz, " Added by akmadasu for CHG0191529
           lv_ktokk  TYPE ktokk.
...
FORM f_format_bdc_accntng_item
  USING    iwa_bbseg TYPE ty_wa_bbseg.
  data: lv_MWSKZ type MWSKZ. " Added BY AKMADASU FOR CHG0191529
  data: lv_hkont type hkont. " Added BY AKMADASU FOR CHG0191529
* Screen: Create G/L account item
  PERFORM  bdc_screen   USING     'SAPMF05A'     '0300'.
  PERFORM  bdc_field    USING     'BSEG-WRBTR'   iwa_bbseg-wrbtr.
**--START OF CHNAGES BY AKMADASU FOR CHG0191529
*  PERFORM  bdc_field    USING     'BSEG-MWSKZ'   iwa_bbseg-mwskz.
  clear: lv_MWSKZ,lv_hkont.
  lv_hkont = iwa_bbseg-hkont.
  replace all OCCURRENCES OF '#' in lv_hkont with ' '.
  ...
  if sy-subrc is INITIAL.
    SELECT single MWSKZ from SKB1 into lv_MWSKZ WHERE bukrs = gv_bukrs
                                                and   SAKNR = lv_hkont.
...
*    MOVE     iv_nbr_doc               TO lwa_errs_data_dup-nbr_doc.
    read TABLE GIT_BDCDATA into gwa_bdcdata with key fnam = 'BKPF-XBLNR'.
    IF sy-subrc is INITIAL.
      MOVE     gwa_bdcdata-fval            TO lwa_errs_data_dup-nbr_doc.
    ENDIF.
**--end OF CHNAGES BY AKMADASU FOR CHG0191529
...
*    MOVE     iv_nbr_doc             TO lwa_docs_posted-nbr_doc.
    read TABLE GIT_BDCDATA into gwa_bdcdata with key fnam = 'BKPF-XBLNR'.
    IF sy-subrc is INITIAL.
      MOVE     gwa_bdcdata-fval            TO lwa_docs_posted-nbr_doc.
    ENDIF.
**--end OF CHNAGES BY AKMADASU FOR CHG0191529
...
**--START OF CHANGES BY AKMADASU FOR CHG0191529
*              048 text-r36.
              052 text-r36.
**--END OF CHANGES BY AKMADASU FOR CHG0191529
      CLEAR  lwa_docs_posted.
    ENDLOOP.
...
*      WRITE: /001 lwa_errs_data-text(120).
              052 text-r54,        056 lwa_errs_data-rc,
              076 text-r55,        072 lwa_errs_data-msgid,
              107 text-r56,        103 lwa_errs_data-msgty,
              119 text-r57,        115 lwa_errs_data-msgno.
      WRITE: /001 lwa_errs_data-text(124).
**--END OF CHANGES BY AKMADASU FOR CHG0191529
      CLEAR  lwa_errs_data.
    ENDLOOP.
...
*      WRITE: /001 lwa_errs_proc-text(120).
              ...
              052 text-r54,        056 lwa_errs_proc-rc,
              064 text-r55,        072 lwa_errs_proc-msgid,
              095 text-r56,        103 lwa_errs_proc-msgty,
              107 text-r57,        115 lwa_errs_proc-msgno.
      WRITE: /001 lwa_errs_proc-text(124).
**--END OF CHANGES BY AKMADASU FOR CHG0191529
      CLEAR  lwa_errs_proc.
    ENDLOOP.
...
*    MOVE     iv_nbr_doc               TO lwa_errs_data-nbr_doc.
  read TABLE GIT_BDCDATA into gwa_bdcdata with key fnam = 'BKPF-XBLNR'.
  IF sy-subrc is INITIAL.
    MOVE     gwa_bdcdata-fval            TO lwa_errs_data-nbr_doc.
  ENDIF.
**--end OF CHNAGES BY AKMADASU FOR CHG0191529
...
*    MOVE     iv_nbr_doc               TO lwa_errs_proc-nbr_doc.
  read TABLE GIT_BDCDATA into gwa_bdcdata with key fnam = 'BKPF-XBLNR'.
  IF sy-subrc is INITIAL.
    MOVE     gwa_bdcdata-fval            TO lwa_errs_proc-nbr_doc.
  ENDIF.
**--end OF CHNAGES BY AKMADASU FOR CHG0191529
