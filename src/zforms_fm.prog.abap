...
*      value of excel-cell
TYPES: ty_d_itabvalue             TYPE zfi_alsmex_tabline-value,

*      Begin of changes for EMEA Wave0
*      internal table containing the excel data
*       ty_t_itab                  TYPE alsmex_tabline   OCCURS 0,
        ty_t_itab                  TYPE zfi_alsmex_tabline   OCCURS 0,
*      End of changes for EMEA Wave0
...
*  <--  p2        text
*----------------------------------------------------------------------*
FORM separated_to_intern_convert1 TABLES i_tab       TYPE ty_t_sender
  ...
  LOOP AT i_tab.
    ...
    WHILE i_tab CA i_separator.
      ...
      PERFORM line_to_cell_separat1 TABLES i_intern
    ...
    ENDIF.
  ENDLOOP.
ENDFORM.                    " SEPARATED_TO_INTERN_CONVERT1
*---------------------------------------------------------------------*
FORM line_to_cell_separat1 TABLES i_intern    TYPE ty_t_itab
                          USING  i_line
                                 i_row       LIKE sy-tabix
...
* csv Dateien mit separator in Zelle: --> ;"abc;cd";
  IF ( i_separator = ';' OR  i_separator = ',' ) AND
       l_string(1) = gc_esc.
    PERFORM line_to_cell_esc_sep1 USING l_string
                                       l_sic_int
                                       i_separator
                                       i_intern-value.
  ELSE.
...
ENDFORM.                    "line_to_cell_separat1

*---------------------------------------------------------------------*
FORM line_to_cell_esc_sep1 USING i_string
                                i_sic_int      TYPE i
                                i_separator    TYPE c
