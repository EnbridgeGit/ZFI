*&---------------------------------------------------------------------*
*& Report  ZLSDI004_CONTRAX_SO_IDOC_TST                                *
*&                                                                     *
*&---------------------------------------------------------------------*
*&**********************************************************************
...
*       the file created by program ZLSDC005_CONTRAX_SO_TRANSLATE.     *
*                                                                      *
*&---------------------------------------------------------------------*
*CHANGES****                                                           *
* 11/04/2016 JRHARTUNG D30K927432 ACR-2583 - Cap & Trade Enhancements  *
* 12/04/2016 GYMANA    D30K927651 ACR-244  - Segment E1EDK14 Bug fix   *
* 01/18/2017 GYMANA    D30K927837 ACR-244  - Header Key fix            *
* 03/06/2017 GYMANA    D30K928011 ACR-2583 - Add PRSDT as header key   *
*                                            field.                    *
*&---------------------------------------------------------------------*
*&---------------------------------------------------------------------*

REPORT  ZLSDI004_CONTRAX_SO_IDOC_TEST LINE-SIZE 260.
" LINE-COUNT 65
" NO STANDARD PAGE HEADING.

* Intput file format
DATA:  BEGIN OF IDOCREC,                                    "D30K927432
...
*       KBETHDR(11)           TYPE C,    "Header Condition Amount
       ...
       END   OF IDOCREC.                                    "D30K927432

DATA:  BEGIN OF INTAB OCCURS 0,                             "D30K927432
        PARTN(17)             TYPE C,    "Customer Number
        BSART(4)              TYPE C,    "Sales Document Type
        DWERK(4)              TYPE C,    "Delivering Plant
        VKBUR(4)              TYPE C,    "Sales Office
        ...
        KDKG1(2)              TYPE C,    "Material Pricing Group
        KVGR2(3)              TYPE C,    "Customer Group 2
        PRSDT                 TYPE D,    "Pricing Date      "D30K928011
        MABNR(18)             TYPE C,    "Material Number
        ...
        VRKME(3)              TYPE C,    "Unit of Measure
        KSCHL(4)              TYPE C,    "Condition Type
       END   OF INTAB.                                      "D30K927432

DATA:  BEGIN OF SUMTAB OCCURS 0,                            "D30K927432
        PARTN(17)             TYPE C,    "Customer Number
        BSART(4)              TYPE C,    "Sales Document Type
        DWERK(4)              TYPE C,    "Delivering Plant
        VKBUR(4)              TYPE C,    "Sales Office
...
INITIALIZATION.
  CONCATENATE '/usr/sap/interfaces/' SY-SYSID+0(3)
              '/IFFI061/zcontraxsoidoc.dat' INTO INFILE.
...
* SORT INTAB BY PARTN BSART DWERK VKBUR AUGRU ZUONR KONDA   "D30K927432
*               KDGRP MABNR KVGR1 KDKG1.                    "D30K927432
*
*  SORT INTAB BY PARTN BSART DWERK VKBUR AUGRU ZUONR KONDA   "D30K927432
*                KDGRP KVGR1 KDKG1 KVGR2 MABNR.              "D30K927432

  SORT INTAB BY PARTN BSART DWERK VKBUR AUGRU ZUONR KONDA   "D30K928011
                KDGRP KVGR1 KDKG1 KVGR2 PRSDT MABNR.        "D30K928011
...
*       AT END OF KDKG1.                                    "D30K927432
    AT END OF MABNR.                                        "D30K927432
      SUM.
  ...
  ENDLOOP.
**
** SORT SUMTAB BY PARTN BSART DWERK VKBUR AUGRU ZUONR KONDA  "D30K927432
**                KDGRP KVGR1.                               "D30K927432
**
** SORT SUMTAB BY PARTN BSART DWERK VKBUR AUGRU ZUONR KONDA  "D30K927432
**                KDGRP KVGR1 KDKG1 KVGR2 MABNR.             "D30K927432
**
  SORT SUMTAB BY PARTN BSART DWERK VKBUR AUGRU ZUONR KONDA  "D30K928011
                 KDGRP KVGR1 KDKG1 KVGR2 PRSDT MABNR.       "D30K928011
**
***start of temp code
** LOOP AT SUMTAB.
...
**Start of HEADER1
**   AT NEW KDGRP.
**   AT NEW KVGR1.                                           "D30K927432
**    AT NEW KVGR2.                                          "D30K927432
    AT NEW PRSDT.                                           "D30K928011
...
**ACR-244 D30K927651 Change Segment 6 & 7 mapping
      SEGMENT_E1EDK14:
      '008' SUMTAB_LINE-VKORG, '006' SUMTAB_LINE-SPART,     "D30K927651
      '007' SUMTAB_LINE-VTWEG, '012' SUMTAB_LINE-BSART,     "D30K927651
...
**End of HEADER8
**
**Start of HEADER8        (Using Macros) - Customer Group 2  "D30K927432
      IF       ( SUMTAB_LINE-KVGR2    IS NOT INITIAL ).     "D30K927432
        CLEAR                            HEADER8.           "D30K927432
        MOVE     '002'                TO HEADER8-QUALZ.     "D30K927432
        MOVE     SUMTAB_LINE-KVGR2    TO HEADER8-CUSADD.    "D30K927432
        PERFORM  ADD_SEGMENT      TABLES INT_EDIDD          "D30K927432
                                   USING CONTROL_RECORD-DOCNUM "K927432
                                         'E1EDK35'          "D30K927432
                                         HEADER8.           "D30K927432
      ENDIF.                                                "D30K927432
**End of HEADER8                                             "D30K927432
**
    ENDAT.   "KDGRP.
**
**Start of ITEM1
...
**    AT END OF KDGRP.
**     AT END OF KVGR1.                                     "D30K927837
**     AT END OF KVGR2.                                     "D30K927837
    AT END OF PRSDT.                                        "D30K928011
      CLEAR W_ITEMNO.
      PERFORM IDOC_SEND_ASYNC TABLES INT_EDIDD USING CONTROL_RECORD.
      CLEAR INT_EDIDD.
      REFRESH INT_EDIDD.
