REPORT zlsdr006 NO STANDARD PAGE HEADING LINE-COUNT 65 LINE-SIZE 132.
...
TABLES: zglcond_ctx,      "GL / Condition Type Mapping Table
        zlsdc01,          "Contrax Gas Sales Order Mapping Table
        t685,             "Conditions Type Table
        t685t,            "Condition Type Name Table
        a004,             "Material Table
        a032,             "Price Group/Material Table
        a999,             "Sales org./Distr. Chl/Material/District Table
...
PARAMETER: infile LIKE filenameci-fileextern DEFAULT
        '/usr/sap/interfaces/P01/LSDCNTXDP/zcontraxdpsordact.dat'.
...
DATA:  BEGIN OF wa_cond OCCURS 0,
         kschl    LIKE A032-kschl,
         vtweg    LIKE A032-vtweg,
*         matnr    LIKE A032-matnr,
         datab    LIKE A032-datab,
         datbi    LIKE A032-datbi,
         knumh    LIKE A032-knumh,
       END OF wa_cond.
...
DATA:  inrec(400),
       ...
       w_desc(40)                 TYPE c,
       w_matnr                    TYPE zlsdc01-matnr,
       w_kozgf                    TYPE t685-kozgf,
       msg_text(50).
...
* Select Material number from ZLSDC01
  CLEAR w_matnr.
  ...
  IF sy-subrc <> 0.
    ...
    MOVE 'Material number not found in table ZLSDC01.'
         TO err_file-err_msg.
    APPEND err_file.
...
FORM process-split-records.
  ...
  LOOP AT itab_glcondctx.
    ...
    SELECT kozgf INTO w_kozgf
      FROM T685
     WHERE kschl = itab_glcondctx-kschl.
    ENDSELECT.

    SELECT SINGLE vtext
      INTO itab_condtype-vtext
      FROM t685t
     WHERE kschl = itab_glcondctx-kschl.
    ...
    IF w_kozgf = 'K004'.
       SELECT SINGLE kschl vtweg datab datbi knumh
         INTO wa_cond
         FROM A004
        WHERE kschl = itab_glcondctx-kschl
    ...
    IF sy-subrc = 0.
      SELECT * FROM konp
       WHERE knumh = wa_cond-knumh
         AND kschl = itab_glcondctx-kschl
         AND loevm_ko <> 'X'.

        IF sy-subrc = 0.
          MOVE wa_cond-knumh to itab_condtype-knumh.
          MOVE wa_cond-vtweg to itab_condtype-vtweg.
          MOVE wa_cond-kschl to itab_condtype-kschl.
          MOVE wa_cond-datab to itab_condtype-datab.
          MOVE wa_cond-datbi to itab_condtype-datbi.
...
*          itab_condtype-vtext = itab_glcondctx-vtext.
        ...
        ELSE.
          ...
          CONCATENATE crec-app_yr crec-app_mth '01'
                 INTO err_file-eff_date.
          MOVE 'No cond. type detail records found in table KONP.'
           TO err_file-err_msg.
          APPEND err_file.
          MOVE 'Y' TO w_errfile_created_flag.
          MOVE 'Y' TO w_ctype_not_found_flag.
    ...
    ELSE.                                                   "SDP41704
      ...
      CONCATENATE crec-app_yr crec-app_mth '01'
             INTO err_file-eff_date.
      IF w_kozgf = 'K004'.
         MOVE 'No cond. type detail records found in table A004.'
           TO err_file-err_msg.
      ELSEIF w_kozgf = 'K032'.
         MOVE 'No cond. type detail records found in table A032.'
           TO err_file-err_msg.
      ELSEIF w_kozgf = 'ZTSD'.
         MOVE 'No cond. type detail records found in table A999.'
           TO err_file-err_msg.
      ENDIF.
      APPEND err_file.
      MOVE 'Y' TO w_errfile_created_flag.
...
* sum up dollar amounts.
    ...
    MOVE itab_condtype-vtext to splttab-vtext.
