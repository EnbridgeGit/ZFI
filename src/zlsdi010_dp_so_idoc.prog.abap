...
*       the file created by program ZLSDC016_DP_SO_TRANSLATE.          *
*                                                                      *
*&---------------------------------------------------------------------*
*CHANGES****                                                           *
*& 12/04/2016 GYMANA D30K927651 ACR-244 - Segment E1EDK14 Bug fix      *
*&---------------------------------------------------------------------*
*& 2021/05/17 wyachalj COG changes to include Entrac on Selection Screen
*&---------------------------------------------------------------------*

REPORT  ZLSDI010_DP_SO_IDOC LINE-SIZE 260.
...
INITIALIZATION.
  CONCATENATE '/usr/sap/interfaces/' SY-SYSID+0(3)
              '/LSDCNTXDP/zcontraxdpsoidoc.dat' INTO INFILE.
...
*Note: Both values OWN_LOGICAL_SYSTEM or 'SAPQ02' as sendr port work OK
  ...
  IF P_ENT = 'X'.                               " for ENTRAC
    CONTROL_RECORD-SNDPRN = 'ENTGS'.             "Partner # of Sender
  ELSE.                                         " for CONTRAX
