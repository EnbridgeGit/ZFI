...
FORM new_entry_validation.
  ...
  zfimv_doa_new-changedon = ltp_timestamp.
  zfimv_doa_new-changedby = ltp_username.
  ...
  IF zfimv_doa_new-lookup_key_type = 'E'.
    ltp_pernr = zfimv_doa_new-lookup_key.
    CALL FUNCTION 'ZFI_GET_USER_DETAILS' DESTINATION ltp_log_dest
      EXPORTING
        imp_pernr             = ltp_pernr
      IMPORTING
        exp_vorna             = zfimv_doa_new-vorna
        exp_nachn             = zfimv_doa_new-nachn
      EXCEPTIONS
        system_failure        = 1
        communication_failure = 2.
  ELSEIF zfimv_doa_new-lookup_key_type = 'G'.
