REPORT ZFGLR024B NO STANDARD PAGE HEADING MESSAGE-ID ZS
                                         LINE-COUNT 65
                                         LINE-SIZE  120.
...
*  This program is a copy of ZFGLR024 & changes are made for variants, *
*  DB Tables, Dynamic Where clause and fields to download.             *
*                                                                      *
************************************************************************
*CHANGES:                                                              *
*Issue   By      Date    Description                                   *
*TR859  M. Khan 09/17/2010 Add Ledger Group AL with group Blank in     *
*                          in Dynamic Where clause.                    *
*TR896  G.Ymana 2011/01/18 Change all dollar fields to handle larger   *
*                          amounts.                                    *
*                                                                      *
*                                                                      *
*----------------------------------------------------------------------*
...
TABLES:  BKPF,    "BKPF Accounting Document Header
         ...
         BSEG_ADD,"BSEG_ADD Entry View of Acct Doc for Additionl Ledgers
...
*      COND(32)  TYPE C,                                   "TR859
      COND(40)  TYPE C,                                    "TR859
      END OF WHERE_CLAUSE.
...
*       TOT_DOLLARS  LIKE BSEG-DMBTR,
       DET_DOLLARS  TYPE P DECIMALS 2,
       COMP_DOLLARS TYPE P DECIMALS 2,
       TOT_DOLLARS  TYPE P DECIMALS 2,
...
SELECTION-SCREEN BEGIN OF BLOCK A1 WITH FRAME TITLE TEXT-TL1.
PARAMETERS:
...
SELECTION-SCREEN BEGIN OF BLOCK A2 WITH FRAME TITLE TEXT-TL2.
PARAMETER:
  P_LDGR1  RADIOBUTTON    GROUP REPT,
  P_LDGR2  RADIOBUTTON    GROUP REPT,
  P_LDGR3  RADIOBUTTON    GROUP REPT,
  P_LDGR4  RADIOBUTTON    GROUP REPT.
SELECTION-SCREEN END OF BLOCK A2.
SELECTION-SCREEN BEGIN OF BLOCK A3 WITH FRAME TITLE TEXT-TL3.
PARAMETERS:
...
PARAMETERS:
  TUCHFILE      LIKE RFPDO-RFBIFILE.                      "File name
...
CONCATENATE '/usr/sap/interfaces/' SY-SYSID+0(3) '/IFFI063/'
              W_LOCATION 'gl_detail.txt' INTO P_FILE.

CONCATENATE '/usr/sap/interfaces/' SY-SYSID+0(3) '/IFFI063/'
              W_LOCATION 'gl_sum.csv' INTO CSVFILE.
        ...
        WHEN P_TUCH.
             LOOP AT SCREEN.
                IF SCREEN-NAME = 'TUCHFILE'.
                  ...
                  '/IFFI063/' W_LOCATION 'gl_intrnl.tch' INTO TUCHFILE.
                   MODIFY SCREEN.
                ENDIF.
             ENDLOOP.
...
FORM BUILD_WHERE_CLAUSE.
     CONCATENATE 'BUKRS' ' IN ' 'S_BUKRS' INTO WHERE_CLAUSE-COND
     SEPARATED BY SPACE.
     APPEND WHERE_CLAUSE.
     CLEAR WHERE_CLAUSE-COND.

     CONCATENATE 'AND' 'GJAHR' ' = '  'P_YEAR'  INTO WHERE_CLAUSE-COND
     SEPARATED BY SPACE.
     APPEND WHERE_CLAUSE.
     CLEAR WHERE_CLAUSE-COND.

     CONCATENATE 'AND' 'BUDAT' ' IN ' 'S_BUDAT' INTO WHERE_CLAUSE-COND
     SEPARATED BY SPACE.
     APPEND WHERE_CLAUSE.
     CLEAR WHERE_CLAUSE-COND.

IF P_LDGR1 = 'X'.
     CONCATENATE 'AND' 'LDGRP' ' = ' '''0L''' INTO WHERE_CLAUSE-COND
     SEPARATED BY SPACE.
     APPEND WHERE_CLAUSE.
     CLEAR WHERE_CLAUSE-COND.
     CONCATENATE 'AND' 'BSTAT' ' = '  ''' '''   INTO WHERE_CLAUSE-COND
     SEPARATED BY SPACE.
     APPEND WHERE_CLAUSE.
     CLEAR WHERE_CLAUSE-COND.
ELSEIF P_LDGR2 = 'X'.
     CONCATENATE 'AND' 'LDGRP' ' = ' '''NL''' INTO WHERE_CLAUSE-COND
     SEPARATED BY SPACE.
     APPEND WHERE_CLAUSE.
     CLEAR WHERE_CLAUSE-COND.
     CONCATENATE 'AND' 'BSTAT' ' = '  '''L'''   INTO WHERE_CLAUSE-COND
     SEPARATED BY SPACE.
     APPEND WHERE_CLAUSE.
     CLEAR WHERE_CLAUSE-COND.
ELSEIF P_LDGR3 = 'X'.
* CONCATENATE 'AND' 'LDGRP' ' = ' ''' ''' INTO WHERE_CLAUSE-COND "TR859
*     SEPARATED BY SPACE.                                        "TR859
*     APPEND WHERE_CLAUSE.                                       "TR859
*     CLEAR WHERE_CLAUSE-COND.                                   "TR859

     CONCATENATE 'AND' 'LDGRP' ' IN ' '('' '', ''AL'')'          "TR859
                  INTO WHERE_CLAUSE-COND SEPARATED BY SPACE.     "TR859
     APPEND WHERE_CLAUSE.                                        "TR859
     CLEAR WHERE_CLAUSE-COND.                                    "TR859
     ...
     CLEAR WHERE_CLAUSE-COND.
ELSE.
*    CONCATENATE 'AND' 'LDGRP' ' IN ' '('' '', ''0L'', ''NL'')'  "TR859
     CONCATENATE 'AND' 'LDGRP' ' IN ' '('' '', ''0L'', ''AL'', ''NL'')'
                  INTO WHERE_CLAUSE-COND SEPARATED BY SPACE.
     APPEND WHERE_CLAUSE.
     CLEAR WHERE_CLAUSE-COND.

     CONCATENATE 'AND' 'BSTAT' ' IN '  '('' '', ''L'')'
                  INTO WHERE_CLAUSE-COND SEPARATED BY SPACE.
     APPEND WHERE_CLAUSE.
     CLEAR WHERE_CLAUSE-COND.
...
FORM GET_DB_DATA.
  ...
  SELECT BUKRS BELNR BKTXT MONAT GJAHR CPUDT BUDAT USNAM KURSF
         BVORG DBBLG STBLG STJAH XBLNR BLART WAERS LDGRP
  ...
  IF P_LDGR1 = 'X' OR P_LDGR3 = 'X' OR P_LDGR4 = 'X'.
     IF BKPF-LDGRP <> 'NL'.
        CLEAR BSEG.
...
DATA: W_WRBTR(15)  TYPE C,
      ...
      CASE TRUE.
           WHEN P_LDGR1.
                CONCATENATE TEXT-040 TEXT-041 INTO W_TEXT02.
           WHEN P_LDGR2.
                CONCATENATE TEXT-040 TEXT-042 INTO W_TEXT02.
           WHEN P_LDGR3.
                CONCATENATE TEXT-040 TEXT-043 INTO W_TEXT02.
           WHEN P_LDGR4.
                CONCATENATE TEXT-040 TEXT-044 INTO W_TEXT02.
           WHEN OTHERS.
      ENDCASE.
      WRITE W_TEXT02 TO W_TEXT02 CENTERED.
...
*Append header line to the file
  CONCATENATE TEXT-110 TEXT-011 TEXT-012 TEXT-013 TEXT-014
  ...
  LOOP AT ITAB.
       ...
       CONCATENATE ITAB-LDGRP ITAB-BELNR ITAB-BKTXT ITAB-MONAT
        ITAB-GJAHR ITAB-CPUDT ITAB-BUDAT ITAB-USNAM ITAB-NAME_TEXT
...
FORM CREATE_TOUCH_FILE.
     OPEN DATASET TUCHFILE FOR OUTPUT IN TEXT MODE ENCODING DEFAULT
                                            MESSAGE MSG.
        IF SY-SUBRC NE '0'.
...
FORM OPEN_SUMMRY_FILE.
  OPEN DATASET CSVFILE FOR OUTPUT IN TEXT MODE ENCODING DEFAULT
               MESSAGE MSG.
  IF SY-SUBRC NE '0'.
...
FORM WRITE_STATISTICS.
     ...
     WRITE TEXT-108 TO W_TEXT02 CENTERED.
     CLEAR W_TEXT03.
     NEW-PAGE.
     LOOP AT COMPTAB.
...
TOP-OF-PAGE.
    ...
    WRITE: / TEXT-CLT UNDER TEXT-RPT, SY-MANDT, SY-SYSID,    "Client
             28 W_TEXT02, 93 TEXT-PGE, SY-PAGNO.             "Page
    ULINE.
