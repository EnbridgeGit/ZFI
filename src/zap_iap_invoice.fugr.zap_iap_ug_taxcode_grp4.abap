FUNCTION ZAP_IAP_UG_TAXCODE_GRP4.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*                            Spectra Energy                            *
************************************************************************
*  Function Module:  ZAP_IAP_UG_TAXCODE_GRP4                           *
...
*  Date    By        Description                                       *
* -------- --------- ------------------------------------------------- *
* 08/24/18 VRAJAPUTRA D30K928897 - CHG0109670 - Initial Development    *
*                                                                      *
*----------------------------------------------------------------------*
************************************************************************
...
*      IF ls_mwdat-msatz gt 0.
      lv_msatz = lv_msatz + ABS( ls_mwdat-msatz ).
*      ENDIF.
  ...
  LOOP AT lt_gl ASSIGNING <lfs_gl>.
    ...
    IF sy-subrc is INITIAL.
      LOOP AT lt_accounttax INTO ls_accounttax WHERE GL_ACCOUNT IS NOT INITIAL. " Direct Taxable line
        <lfs_curr>-amt_doccur =  <lfs_curr>-amt_doccur - (  lv_amount  * ( ls_accounttax-tax_rate / 100 ) ).
      ENDLOOP.
    ENDIF.
...
*--> Fill the AMounts of corresponding TAx Lines
    ...
    IF sy-subrc is INITIAL.
      <lfs_curr>-amt_doccur =   ( lv_amount ) * ( ls_accounttax-tax_rate / 100 ) .
    ENDIF.
  ENDLOOP.
