REPORT ZFFII017A NO STANDARD PAGE HEADING LINE-COUNT 65
                                         LINE-SIZE 120 MESSAGE-ID ZS.
************************************************************************
* This program is a copy of ZFFII017. It's usage is one time only.     *
...
*      W_DOLLARS = W_DOLLARS * -1.                   "One time change
...
*Get all entries total for this account from table GLT0.
 ...
 ENDSELECT.
  ...
  IF  SY-SUBRC = 0.
      ...
      SUBGLT0-AMOUNT = TOT_UMS + LFC1-UMSAV - TOT_UMH. "One time
*      SUBGLT0-AMOUNT = ( TOT_UMS + LFC1-UMSAV - TOT_UMH ) * -1.
      IF SUBGLT0-AMOUNT <> 0.
         VENDOR_LFC1_$ = VENDOR_LFC1_$ + SUBGLT0-AMOUNT. "One time
*         VENDOR_LFC1_$ = VENDOR_LFC1_$ + ( SUBGLT0-AMOUNT * -1 ).
  ...
  IF VENDOR_GLT0_$ <> VENDOR_LFC1_$.
     SUBGLT0-AMOUNT = VENDOR_GLT0_$ - VENDOR_LFC1_$. "One time
*     SUBGLT0-AMOUNT = ( VENDOR_GLT0_$ - VENDOR_LFC1_$ ) * -1.
...
FORM ACCOUNT_BY_DEFAULT.
     ...
     SUBGLT0-AMOUNT = SUBGLT0-AMOUNT * -1.
     COLLECT SUBGLT0.
*     CLEAR  SUBGLT0.
ENDFORM.
*----------------------------------------------------------------------*
...
FORM ADD_THIS_RECORD.
     ...
     SUBGLT0-AMOUNT = SUBGLT0-AMOUNT * -1.   "ONE TIME
