*&---------------------------------------------------------------------*
*&  Include          ZFI_READ_MD_FOR_IS_FORMS
*&---------------------------------------------------------------------*
* ---------------------------------------------------------------------*
* 22-Sep-2021  DADIM    DECK921329 CHG0223752 Long text splitting      *
*                                  into multiple lines                 *
*&---------------------------------------------------------------------*
*&---------------------------------------------------------------------*
*&      Form  BUILD_FCAT
...
* Look for commas in field value then wrap around with double quote
          ...
          ENDIF.
        ENDIF.
*Start of change for CHG0223752 by DADIM
        IF p_tabs EQ 'SOOD'.
          REPLACE ALL OCCURRENCES OF nline IN lv_val WITH space.
          REPLACE ALL OCCURRENCES OF hcr_lf   IN lv_val WITH space.
          REPLACE ALL OCCURRENCES OF cl_abap_char_utilities=>cr_lf+0(1) IN lv_val WITH space. "Remove Carriage return only
          REPLACE ALL OCCURRENCES OF cl_abap_char_utilities=>cr_lf+0(2) IN lv_val WITH space. "Remove Line feed only
          CONDENSE lv_val.
        ENDIF.
*End of change for CHG0223752 by DADIM
...
FORM create_tables_list .
  ...
  value-key = 'AFVC'.
  value-text = 'Operation within an order'.
  APPEND value TO gt_list.
      ...
      IF sy-subrc EQ 0.
        ...
        LOOP AT t_tline ASSIGNING <tline>.
          ...
          IF p_tab EQ 'STXL' AND
            ...
            ELSE.
*Start Of changes by DADIM for CHG0223752
*              CONCATENATE LV_TLINE <TLINE>-TDLINE NLINE INTO LV_TLINE.
              CONCATENATE lv_tline <tline>-tdline INTO lv_tline.
*End Of changes by DADIM for CHG0223752
            ENDIF.
          ELSE.
            IF lv_tabix EQ 1.
              CONCATENATE lv_tline <tline>-tdline INTO lv_tline.
