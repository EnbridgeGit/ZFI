...
PARAMETERS: r_pc RADIOBUTTON GROUP rad2 DEFAULT 'X' USER-COMMAND cmd,
            p_file TYPE rfpdo-lboxfile DEFAULT 'H:\SAPTEMP\AssetLoad.csv', " OBLIGATORY.
...
*CONCATENATE p_sfile 'SAP-' p_gjahr '-AALoad.csv' INTO p_sfile.
...
*
*  IF r_pc = lc_x.
*    p_file = 'H:\SAPTEMP\AssetLoad.csv'.
*  ELSE.
*    CLEAR p_file.
*  ENDIF.
  ...
  LOOP AT lt_anla INTO ls_anla.
    ...
    LOOP AT lt_anlc INTO ls_anlc FROM lv_tabix.
      IF ls_anlc-bukrs <> ls_anla-bukrs OR
        ...
        EXIT.
      ENDIF.
      CHECK ls_anlc-kansw <> 0.
      gs_alv_output-gjahr = ls_anlc-gjahr.
      lv_amount = ls_anlc-kansw.
      if lv_amount < 0.
         gs_alv_output-kansw = lv_amount.
         CALL FUNCTION 'CLOI_PUT_SIGN_IN_FRONT'
           CHANGING
             value         = gs_alv_output-kansw.
      else.
        gs_alv_output-kansw = lv_amount.
      endif.
      CONDENSE  gs_alv_output-kansw NO-GAPS.
      SHIFT  gs_alv_output-kansw RIGHT.
      lv_append = 'X'.
...
*Prepare table for file output
  LOOP AT gt_alv_output INTO gs_alv_output.
    ...
    gs_file_output-asset_ttype   = 'Asset_Load'.
    ...
    gs_file_output-asset_amount  =  gs_alv_output-kansw.
...
TRY.
      io_column ?= io_columns->get_column( 'KANSW' ).
